...
func (s *memfdSource) slice(cacheOff, length int64) ([]byte, error) {
	i := s.findEntry(cacheOff)
	if i < 0 {
		return nil, BytesNotAvailableError{}
	}
	e := s.entries[i]
	offsetInEntry := cacheOff - e.cacheStart
	if offsetInEntry+length > e.size {
		return nil, BytesNotAvailableError{}
	}
	srcOff := e.srcStart + offsetInEntry

	return s.memfd.Bytes()[srcOff : srcOff+length], nil
}
