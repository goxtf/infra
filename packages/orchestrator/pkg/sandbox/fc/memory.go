...
func (p *Process) exportMemoryFromMemfd(
...
) (block.Cacher, *header.DiffMetadata, error) {
	...
	if originalMemfile == nil {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to create MemfdCache: %w", err)
		}

		return cache, nil, nil
	}

	cache, diffMeta, err := block.NewCacheFromMemfdDeduped(ctx, originalMemfile, blockSize, cachePath, memfd, guestRanges)
	if err != nil {
...
func (p *Process) ExportMemory(
	...
	dedupCache, dedupMetadata, err := cache.Dedup(
		ctx,
		originalMemfile,
		include,
		blockSize,
